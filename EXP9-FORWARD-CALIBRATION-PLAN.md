...
```bash
grep -n "AUTHOR REQUIRED\|Calibration assumptions" "[OVERLEAF_PROJECT] — Credibility and Trust in Polymatroidal Service Markets/sections/SmallestDetectableStake.tex"
```
...
```bash
cd "[OVERLEAF_PROJECT] — Credibility and Trust in Polymatroidal Service Markets/"
...
```bash
cd "[USER_HOME]/Dropbox/Research/workspace/5 Support/Groups/FCG/"
...
grep -nE "expL2_forward_calibration|fig:expL2" "[OVERLEAF_PROJECT] — Credibility and Trust in Polymatroidal Service Markets/sections/"*.tex
```
...
```bash
grep -n "AUTHOR.REQUIRED" "[OVERLEAF_PROJECT] — Credibility and Trust in Polymatroidal Service Markets/sections/"*.tex
```
...
```bash
grep -n "Issue 1\|SDS theorem rigor" "[USER_HOME]/Dropbox/Research/workspace/5 Support/Groups/FCG/Review/2026-Trilogy-2-TEAC — Credibility and Trust in Polymatroidal Service Markets (TEAC)/round-1/revision-summary.md"
```
...
- **Operate in the TSC simulation repo** (`[REPO_ROOT]/`), which is git-tracked. The vault symlink `Experiments/credible-marketplace-sim` resolves there.
